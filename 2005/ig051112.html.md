...
今回のリリースの主たる目的は、「一般的なRDBMSでは不要なダブルクオートによるテーブルエスケープ」についての抑制です。この仕様は HSQLDBへの対応のために付与されていたものなのですが、総合的に判断して
「一般的なRDBMSでは不要なダブルクオートによるテーブルエスケープ」は除去するのが妥当であろうとの判断に落ち着きました。
...
結局 JavaOne Tokyoには行くことができませんでした。ちょっと残念です。
聴きたい発表がいろいろあったのですが、まずは目前の仕事を優先して判断しました。
