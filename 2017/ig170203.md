...
このリポジトリが [Sonatype](../keyword/sonatype.html) 的なゴール手前にあたります。`-SNAPSHOT` はここには到達しません。
Maven Repository へのビルドにて初回の場合には、以下のリポジトリで最終確認のうえ [Sonatype](../keyword/sonatype.html) に連絡することにより Maven Repository に到達します。
