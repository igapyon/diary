...
`Branch` ボタンをクリックして、`Switch branches/tags` の下に ブランチ名として `gh-pages` を入力して `Create branch: gh-pages` をクリックします。
...
[github](../keyword/github.html) にログインして、`Settings` タブを開いて `Default branch` から `gh-pages` を選択して `Update` ボタンをクリックします。
