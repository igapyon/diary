...
このドキュメントのゴールは、手元の RDB を OData v4 Server 仕様による REST API Server をすばやく公開できるようになることです。

そして、そのようにローコードで REST API Server を素早く構築するのを支援するツールが [Oiyokan Initializr](https://github.com/igapyon/oiyokan-initializr) なのです。
...
Note: Spring Boot に知見があれば、このログイン認証を容易に書き換えたり除去したりできます。
