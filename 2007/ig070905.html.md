...
次に メールの内容を読み込む＋読んだメールを削除するサンプルを作成しました。
なお、このサンプルは、メール削除の処理が含まれています。サンプル実行に際しては、メールが削除されるという点を予め理解してください。意味が分からない場合には、このサンプルは実行しないでください。

※注意: このサンプルを実行すると、メールサーバ上のメールが削除されます！
明示的に DELETEフラグをセットしています。
